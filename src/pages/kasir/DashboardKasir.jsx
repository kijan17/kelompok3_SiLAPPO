...
const DashboardKasir = () => {
  ...
  useEffect(() => {
    ...
    const kasirId = localStorage.getItem('kasir_id'); // Ambil ID Kasir dulu
    
    if (storedName) setNamaKasir(storedName);

    // KUNCI PERBAIKAN: Cek status shift BERDASARKAN ID KASIR
    ...
    if (shiftActive === 'true' && shiftStart) {
      ...
      // Pastikan di-reset ke 0 jika kasir ini belum mulai shift
  ...
  return (
    <div className="relative min-h-screen font-sans pb-10">
      <div className={`space-y-8 transition-all duration-700 ease-out transform ${isContentMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full max-w-5xl">
          ...
          <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm flex flex-col relative overflow-hidden group hover:border-[#005432]/30 transition-colors">
          ...
          <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm flex flex-col justify-between hover:border-[#005432]/30 transition-colors">
            <div>
            ...
            <div>
              ...
              <p className="text-xs text-center font-medium text-gray-400">
                {isShiftActive ? <span className="text-green-600">Sistem POS Terbuka. Anda sedang dalam sesi shift.</span> : 'Sistem POS Terkunci. Shift belum dimulai.'}
              </p>
            </div>
          </div>
